	...
	return (
		<div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-900">
			<div className="text-xs text-slate-500 font-medium">
				...
				<span className="text-white font-bold">{totalPages}</span> ({totalItems} items total)
			</div>
			...
			<div className="flex items-center gap-2">
				<button
					...
					disabled={page === 0}
					className="btn btn-sm btn-outline border-slate-800 text-slate-300 hover:bg-slate-900"
				>
				...
				<div className="flex gap-1">
					{Array.from({ length: totalPages }).map((_, idx) => {
						...
						if (idx === 1 || idx === totalPages - 2) {
							return <span key={idx} className="text-slate-600 px-1">...</span>;
						}
						return null;
					})}
				</div>
