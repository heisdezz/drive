...
export default function Sidebar() {
	const { selectedDrive, selectDrive, devices, loading, fetchDrives } = useDriveStore();
	...
	return (
		<aside className="w-64 sm:w-72 flex-shrink-0 flex flex-col bg-slate-900 border-r border-slate-950">
			{/* App Brand Header */}
			<div className="p-4 flex items-center justify-between border-b border-slate-950 bg-slate-900/50">
				...
				<div className="space-y-2">
					...
					<nav className="flex flex-col gap-1">
						<Link
							to="/"
							activeProps={{
								className: "bg-slate-800 text-white font-bold border-l-2 border-primary pl-3.5",
							}}
							inactiveProps={{
								className: "text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 pl-4",
							}}
					...
					<div className="space-y-2">
						{loading && devices.length === 0 ? (
							<div className="px-3 py-4 text-center">
								...
								<p className="text-[10px] text-slate-600 mt-1.5">Scanning filesystems...</p>
							</div>
						) : devices.length === 0 ? (
									...
									<div
										...
										className={`p-3 rounded-xl transition-all duration-150 flex flex-col gap-2 border ${
											isSystemRoot
												...
												: "bg-slate-950/40 border-slate-950 hover:bg-slate-950/70 hover:border-slate-900 cursor-pointer"
										}`}
									>
			...
			{/* Sidebar Footer */}
			<div className="p-3 border-t border-slate-950 bg-slate-900/50 flex flex-col gap-2">
				<button 
