...
function AlbumCard({ album, drivePath }: AlbumCardProps) {
		...
		const loadThumb = async () => {
			...
			try {
				const res = await rpc.request.getThumbnail({
					mediaId: preview.id,
					relativePath: preview.current_relative_path,
					drivePath: drivePath,
				...
				console.error("Failed to load album preview thumbnail:", err);
			} finally {
	...
	return (
		...
		>
			<div className="relative w-16 h-16 rounded-xl overflow-hidden bg-slate-900 border border-slate-850 flex-shrink-0 flex items-center justify-center">
				{thumbUrl ? (
					<img
						src={thumbUrl}
						alt={displayName}
...
function Index() {
	const { selectedDrive, devices, loading: loadingDrives, selectDrive, fetchDrives } = useDriveStore();
			...
			<div className="space-y-8">
					...
					<div className="hero-content text-left justify-start p-0 max-w-3xl">
						<div>
							<h2 className="text-4xl sm:text-5xl font-black tracking-tight text-white mb-4 leading-tight">
								Welcome to <span className="bg-clip-text text-transparent bg-gradient-to-r from-primary via-indigo-400 to-secondary">Antigravity Drive</span>
							</h2>
							<p className="text-slate-300 text-lg mb-8 leading-relaxed">
								An ultra-lightweight desktop media library & photo explorer powered by React, Electrobun, and Zig. Connect and select a storage volume to automatically catalog, preview, and search your photo albums.
							</p>
						</div>
					</div>
				</div>
				...
				<section className="space-y-6">
					<div className="flex items-center justify-between border-b border-slate-900 pb-3">
						<h3 className="text-sm font-black tracking-wider text-slate-400 uppercase">Select a Connected Drive</h3>
						{loadingDrives && <span className="loading loading-spinner loading-xs text-primary"></span>}
					</div>
					...
					{nonSystemDevices.length === 0 ? (
						<div className="p-12 text-center rounded-2xl bg-slate-900/20 border border-slate-900/50">
							...
							<p className="text-slate-500 text-xs mt-1.5 max-w-sm mx-auto">
								Connect a storage drive or click "Mount Drive" in the sidebar to load and inspect catalog files.
							</p>
						</div>
					) : (
								...
								return (
									<div
										...
										className={`p-5 rounded-2xl border transition-all duration-300 flex flex-col gap-4 relative overflow-hidden ${
											isMounted
												? "bg-slate-900/35 border-slate-900 hover:border-primary/50 hover:bg-slate-900/60 hover:scale-[1.02] cursor-pointer"
									...
									>
										...
										<div className="flex items-start justify-between">
											<div className="flex items-center gap-3">
												...
												<div>
													<h4 className="font-extrabold text-white text-sm tracking-tight leading-snug truncate max-w-[120px]" title={device.name}>
														{device.name}
													</h4>
										...
										{isMounted ? (
											...
											<div className="flex items-center justify-between mt-4 bg-slate-900/40 p-2.5 rounded-xl border border-slate-900/60">
												<span className="text-[10px] text-slate-500 font-medium">Mount required to explore</span>
												<button
	...
	const sortedAlbums = [...albums].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 4);
		...
		<div className="space-y-8">
				...
				<div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
					...
					<div className="flex items-center gap-2">
						<button
							...
							title="Refresh view"
						>
							<RefreshCw className={`w-3.5 h-3.5 ${loadingMedia || loadingAlbums ? "animate-spin text-primary" : "text-slate-400"}`} />
						</button>
						<button
			...
			<section className="space-y-4">
				<div className="flex items-center justify-between border-b border-slate-900 pb-2.5">
					<h3 className="text-sm font-black tracking-wider text-slate-400 uppercase">Recently Added Media</h3>
					<Link
						to="/medias"
				...
				{loadingMedia && mediaItems.length === 0 ? (
					<div className="flex flex-col items-center justify-center py-16">
						...
						<p className="text-xs text-slate-500 font-semibold">Querying catalog database...</p>
					</div>
				) : mediaItems.length === 0 ? (
					<div className="p-12 text-center rounded-2xl bg-slate-900/10 border border-slate-900 border-dashed">
						<ImageIcon className="w-12 h-12 text-slate-800 mx-auto mb-4" />
						<h3 className="text-white font-bold">No Media Found</h3>
						<p className="text-slate-500 text-xs mt-1.5 max-w-sm mx-auto">
							This storage volume has not been cataloged yet. Switch to the Discover tab to scan files.
						</p>
						<Link
							to="/discover"
			...
			<section className="space-y-4">
				<div className="flex items-center justify-between border-b border-slate-900 pb-2.5">
					<h3 className="text-sm font-black tracking-wider text-slate-400 uppercase">Recent Albums</h3>
					<Link
						to="/albums"
						className="text-xs font-bold text-slate-400 hover:text-primary transition-colors flex items-center gap-1"
					>
				...
				{loadingAlbums && albums.length === 0 ? (
					<div className="flex flex-col items-center justify-center py-12">
						<Loader2 className="w-8 h-8 text-primary animate-spin mb-3" />
						<p className="text-xs text-slate-500 font-semibold">Loading albums...</p>
					</div>
					...
					<div className="p-12 text-center rounded-2xl bg-slate-900/10 border border-slate-900 border-dashed">
						...
						<p className="text-slate-500 text-xs mt-1.5 max-w-sm mx-auto">
							Media folders have not been mapped to albums. Scanning folders will auto-group them into albums.
						</p>
					</div>
				) : (
